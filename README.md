...
- Includes binary dependencies for LevelDB and compression (libcrc32c, libsnappy, libz, libzstd, libleveldb)
...
### ImageCodecC (Internal Target)

- Provides incremental PNG and JPEG scanline encoding
- Encapsulates the bundled libpng, zlib, and libjpeg-turbo binary dependencies
- Exposed to consumers through the Swift `CBImageScanlineWriter` API
...
libpng is built against the bundled libz output and linked internally through `ImageCodecC`.
