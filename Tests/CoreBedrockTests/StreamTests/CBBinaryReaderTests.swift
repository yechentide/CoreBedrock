...
struct CBBinaryReaderTests {
    ...
    @Test
    func testReadIntegersLittleEndian() throws {
        // UInt8
        ...
        #expect(try reader.readInt32() == -1985229329)
        
        // UInt64
        reader = CBBinaryReader(bytes: [0xBC, 0x9A, 0x78, 0x56, 0x34, 0x12, 0xF0, 0xDE]) // 0xDEF0123456789ABC
        #expect(try reader.readUInt64() == 0xDEF0123456789ABC)

        // Int64
        reader = CBBinaryReader(bytes: [0x45, 0x23, 0x01, 0xEF, 0xCD, 0xAB, 0x89, 0xF7]) // 0xF789ABCDEF012345
        #expect(try reader.readInt64() == Int64(bitPattern: 0xF789ABCDEF012345))
    }

    @Test
    func testReadIntegersBigEndian() throws {
        // UInt16
        ...
        // UInt64
        reader = CBBinaryReader(bytes: [0xDE, 0xF0, 0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC], littleEndian: false) // 0xDEF0123456789ABC
        #expect(try reader.readUInt64() == 0xDEF0123456789ABC)

        // Int64
        reader = CBBinaryReader(bytes: [0xF7, 0x89, 0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45], littleEndian: false) // 0xF789ABCDEF012345
        #expect(try reader.readInt64() == Int64(bitPattern: 0xF789ABCDEF012345))
    }

    @Test
    func testReadFloatsLittleEndian() throws {
        // Float (3.1415927)
    ...
    @Test
    func testReadFloatsBigEndian() throws {
        // Float (3.1415927)
