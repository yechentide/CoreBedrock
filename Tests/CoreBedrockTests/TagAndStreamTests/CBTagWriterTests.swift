...
class CBTagWriterTests: XCTestCase {
    ...
    func testErrors() throws {
        ...
        XCTAssertThrowsError(try writer.writeShort(value: 0)) { error in
            XCTAssertEqual(error as! CBStreamError, CBStreamError.invalidFormat("Unexpected tag type (expected: TAG_Int, given: TAG_Short)"))
        }
