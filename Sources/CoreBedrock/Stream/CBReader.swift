...
public class CBReader {
    ...
    private var _nodes: [CBReaderNode]
    ...
    private let erroneousStateError = "CBReader is in an erroneous state!"
    
    /// Initializes a new instance of the `CBReader` class based on the specified stream.
    /// - Parameters:
    ...
    /// Gets or sets whether the `CBReader` should save a copy of the most recently read tag's value. Unless
    ...
    /// Gets whether this `CBReader` instance is in a state of error.
    ...
    /// Parsing option: Whether CBReader should skip End tags in ReadToFollowing() automatically while parsing.
    /// Default is `true`.
    ...
    /// - Throws: A `formatException` error if an error occurred while parsing data in NBT format; an `invalidOperation` error if the `CBReader` cannot recover from a previous parsing error.
    ...
    /// Advances the CBReader to the next descendant tag with the specified name. If a matching child tag is not found, the CBReader is positioned on the end tag.
    ...
    /// Advances the CBReader to the next sibling tag, skipping any child tags. If there are no more siblings, CBReader is positioned on the tag following the last of this tag's descendants.
    /// - Throws: A `formatException` error if an error occurred while parsing data in NBT format; an `invalidOperation` error if the `CBReader` cannot recover from a previous parsing error.
    ...
    /// Advances the CBReader to the next sibling tag with the specified name.
    /// If a matching sibling tag is not found, CBReader is positioned on the tag following the last siblings.
    ...
    /// Returns a String that represents the current CBReader object.
